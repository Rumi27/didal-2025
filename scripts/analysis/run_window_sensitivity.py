...
# LOGGING SETUP
LOG_FILE = "sensitivity_debug.log"
def log(msg):
    try:
...
SHAPEFILE_PATH = Path("Didal_Glacier_GIS_Data/Glacier_Outline/didal_glacier_outline.shp")
...
def load_image(dim_path):
    ...
    # Use _expanded to force new crop
    crop_path = TEMP_DIR / f"crop_{dim_path.stem}_expanded.tif"
...
def get_mask(shapefile_path, geotransform, cols, rows, projection, erode_px):
    ...
    if not source_srs.IsSame(target_srs):
        # log("Reprojecting shapefile layer in memory...")
...
def main():
    log("Starting Main Expanded v2 (Reprojection Support)...")
    
    if not SHAPEFILE_PATH.exists():
        log(f"❌ Shapefile not found: {SHAPEFILE_PATH}")
        sys.exit(1)
    ...
    for i in range(len(products) - 1):
        ...
        roi_mask = ndimage.binary_dilation(roi_mask, iterations=5).astype('uint8')
