#!/usr/bin/env python3
"""
Interactive tool to mark points on Sep 17 image
"""
...
def interactive_measurement():
    """Interactive measurement tool for Sep 17 image"""
    output_dir = Path('planet_images/visualizations')
    image_path = output_dir / 'glacier_cropped_2025-09-17.png'
    ...
    # Load baseline reference point (should be same)
    ...
    ax.imshow(img_array)
    ax.set_title('SEP 17, 2025 (First Movement)\n'
    ...
    # Show baseline reference if available
    if baseline_ref:
    ...
    def on_key(event):
        if event.key == 'enter':
            if points['reference_point'] and points['glacier_front']:
                ...
                output_file = output_dir / 'sep17_measurement.json'
                with open(output_file, 'w') as f:
                    json.dump(results, f, indent=2)
                
                print(f"\n✓ Coordinates saved to: {output_file}")
                print("\nNext: Run python3 interactive_measure_oct25.py")
