...
def measure_image(date_str, date_label, image_path, reference_point=None):
    ...
    # Storage for point and completion flag
    glacier_front = None
    glacier_marker = None
    measurement_complete = threading.Event()
    result_container = {'point': None}
    ...
    if reference_point:
        ...
        ax.text(reference_point[0], reference_point[1] - 30, 'REF', 
               color='green', fontsize=12, fontweight='bold',
               ha='center', va='top',
               bbox=dict(boxstyle='round', facecolor='white', alpha=0.9))
    
    def on_click(event):
        nonlocal glacier_marker, glacier_front
        ...
        if event.button == 3:  # Right click
            ...
            glacier_front = (event.xdata, event.ydata)
            result_container['point'] = glacier_front
            ...
            ax.text(event.xdata, event.ydata - 35, 'GLACIER', 
            ...
            if reference_point:
                ...
                ax.text(0.02, 0.98, f'Distance from ref:\n{dist_m:.1f} m\n({dist_px:.1f} px)', 
    ...
    def on_key(event):
        if event.key == 'enter':
            if result_container['point']:
                print(f"\n✓ Measurement complete for {date_label}")
                measurement_complete.set()
                plt.close(fig)
            else:
    ...
    def on_close(event):
        # If window is closed, check if we have a point
        if result_container['point']:
            measurement_complete.set()
        else:
            print(f"\n⚠ Window closed without marking a point for {date_label}")
            measurement_complete.set()
