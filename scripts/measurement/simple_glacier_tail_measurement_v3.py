#!/usr/bin/env python3
"""
Simple glacier tail measurement - V3 (Most Reliable)
Opens images with system viewer instead of matplotlib
"""
...
def open_image_with_viewer(image_path):
    ...
    try:
        if sys.platform.startswith('linux'):
            ...
            viewers = ['xdg-open', 'eog', 'gimp', 'gwenview', 'feh']
...
def show_image_and_get_coords(date, image_path, label, previous_position=None):
    """
    Open image with system viewer and get coordinates from user
    """
    print("\n" + "="*70)
    ...
    print(f"\n📸 Opening image: {os.path.basename(image_path)}")
    print(f"   Image size: {width} x {height} pixels")
    print(f"   Resolution: {RESOLUTION_M_PER_PIXEL} m/pixel")
    
    if previous_position:
        print(f"\n   Previous position was at: ({previous_position[0]:.1f}, {previous_position[1]:.1f})")
        print(f"   Look for where the glacier tail is now (it has moved)")
    
    if open_image_with_viewer(image_path):
        print("   ✓ Image opened in your default image viewer")
        print("\n   → Look at the image and identify the glacier tail position")
        print("   → You can use an image viewer that shows coordinates (like GIMP, ImageJ)")
        print("   → Or estimate coordinates based on image size")
    else:
...
def main():
    """Main function"""
    print("\n" + "="*70)
    print("SIMPLE GLACIER TAIL MOVEMENT MEASUREMENT - V3")
    print("="*70)
    print("\nThis version opens images with your system image viewer.")
    print("No matplotlib issues - uses your default image viewer!")
