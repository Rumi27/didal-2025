#!/usr/bin/env python3
"""
Automated SNAP processing for same-track Sentinel-1 pairs.

This script automates the processing of 8 same-track pairs using SNAP's
command-line tool (gpt) to calculate glacier velocities for validation.
...
Usage:
    python automated_same_track_processing.py
"""
...
def create_offset_tracking_graph(pair_info):
    """Create SNAP XML graph for offset tracking with 400-pixel search range."""
    graph_id = f"offset_tracking_track{pair_info['track']}_{pair_info['master'].replace('-', '')}_{pair_info['slave'].replace('-', '')}"
    ...
    # Offset Tracking (CRITICAL: 400-pixel search range)
    ...
    ET.SubElement(offset_params, 'windowSizeX').text = '128'
    ET.SubElement(offset_params, 'windowSizeY').text = '128'
